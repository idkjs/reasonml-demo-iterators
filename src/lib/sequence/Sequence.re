...
let rec seq_list_map = (f, l, k) =>
  ...
  | [] => k([])
  | [x, ...tail] =>
    f(x, x' => seq_list_map(f, tail, tail' => k([x', ...tail'])))
  };
...
module MList = {
  ...
  let of_seq = seq => of_seq_with(seq, _ => ());
  ...
  let to_stream = l => Stream.from(_to_next(42, l)); /* 42=magic cookiiiiiie */
...
let persistent_lazy = (seq: t('a)) => {
  ...
  k =>
    ...
    | LazySuspend =>
      ...
      let seq' = MList.of_seq_with(seq, k);
...
let inter = (type a, ~eq=(==), ~hash=Hashtbl.hash, c1, c2) => {
  ...
  c2(x =>
    try(
      ...
      | Inter_left => Tbl.replace(tbl, x, Inter_both) /* save */
      | Inter_both => ()
      }
    ) {
    | Not_found => ()
...
let of_set = (type s, type v, m, set) => {
  module S = (val m: Set.S with type t = s and type elt = v);
  k => S.iter(k, set);
};
...
module Set = {
  ...
  module Adapt = (X: Set.S) : (S with type elt = X.elt and type t = X.t) => {
    ...
    let of_seq = seq => fold((set, x) => X.add(x, set), X.empty, seq);
...
module Map = {
  ...
  module Adapt = (M: Map.S) => {
    let to_seq = m => from_iter(k => M.iter((x, y) => k((x, y)), m));

    let of_seq = seq => fold((m, (k, v)) => M.add(k, v, m), M.empty, seq);
...
module IO = {
  ...
  let write_bytes_to =
      (~mode=0o644, ~flags=[Open_creat, Open_wronly], filename, seq) => {
  ...
  let write_to = (~mode=?, ~flags=?, filename, seq) =>
    write_bytes_to(
      ...
      ~flags?,
      filename,
      map(Bytes.unsafe_of_string, seq),
    );
